...
export const getPosts = (req, res) => {
    ...
    jwt.verify(token, "secretkey", (err, userInfo) => {
        ...
        const q = userId
            ...
            : `SELECT p.*, u.id AS userId, name, profilePic FROM post AS p JOIN users AS u ON (u.id = p.userId)
        ...
        db.query(q, values, (err, data) => {
            ...
            if (!data) return res.status(404).json({ error: "No posts found" });
...
export const addPost = (req, res) => {
    ...
    jwt.verify(token, "secretkey", (err, userInfo) => {
        ...
        db.query(q, [values], (err, data) => {
            if (err) return res.status(500).json({ error: "Database query failed", details: err });
            if (!data) return res.status(404).json({ error: "Failed to create post" });
...
export const deletePost = (req, res) => {
    ...
    jwt.verify(token, "secretkey", (err, userInfo) => {
        ...
        db.query(q, [req.params.id, userInfo.id], (err, data) => {
            if (err) return res.status(500).json({ error: "Database query failed", details: err });
            if (!data || data.affectedRows === 0) return res.status(403).json({ error: "You can delete only your post" });
